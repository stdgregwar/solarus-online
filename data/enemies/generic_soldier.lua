...
-- ADAPTED for online use from zsdx codebase

-- Example of use from an enemy script:

-- local init_enemy = require("enemies/generic_soldier")
-- init_enemy(enemy)
...
return function(enemy)
  ...
  --------------------------------------------------
  ----------- Master Mob Parts ---------------------
  function enemy:on_mob_master_setup()
    function enemy:check_hero()
      local hero = self:get_map():get_nearest_hero(self)
    ...
    function enemy:on_obstacle_reached(movement)
      if being_pushed then
        self:go_hero() if not being_pushed then
      if going_hero then
        self:go_hero()
      else
        self:go_random()
    ...
    function enemy:on_mob_custom_attack_received(attack, sprite,giver)
      local hero = giver
      if attack == "sword" and sprite == sword_sprite then
        sol.audio.play_sound("sword_tapping")
        self:push_back(hero)
      end
      if attack == 'sword' and sprite == main_sprite then
        self:take_normal_damage(hero,attack,1)
      end
    end
    ...
    function enemy:go_hero(hero)
      local hero = hero or self:get_map():get_nearest_hero(self)
