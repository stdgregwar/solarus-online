...
function server_menu:on_started()
...
function server_menu:on_key_pressed(key)
...
function server_menu:on_joypad_button_pressed(button)
...
function server_menu:on_joypad_axis_moved(axis, state)
...
function server_menu:on_joypad_hat_moved(hat, direction8)
...
function server_menu:direction_pressed(direction8)
...
function server_menu:on_draw(dst_surface)
...
function server_menu:draw_server(slot_index)
  ...
  slot.server_name_text:draw(self.surface,87,57+ slot_index * 27)
  slot.server_description:draw(self.surface,87,65 + slot_index * 27)
...
function server_menu:draw_server_cursor()
...
function server_menu:draw_server_number(slot_index)
...
function server_menu:draw_bottom_buttons()
...
function server_menu:read_servers()
  self.slots = {}
  local font, font_size = sol.language.get_dialog_font()
  for i,server in ipairs(serverlist) do
    local slot = {}
    slot.server = server
    slot.file_name = "save_" .. server.host .. ".dat"
    slot.save = sol.game.load(slot.file_name)
    slot.number_img = sol.surface.create("menus/selection_menu_save" .. i .. ".png")

    slot.server_name_text = sol.text_surface.create{
      font = font,
      font_size = font_size
    }
    ...
    local name_text = slot.save:get_value('player_name') or 'new'
    slot.server_name_text:set_text(name_text .. '@' .. server.name)
    network.get_server_header(
      server.host,
      server.port,
      slot.save,
      function(header)
        slot.offline = header.description == 'offline'
        slot.server_description:set_text(header.description)
    end)
...
function server_menu:set_bottom_buttons(key1, key2)
...
function server_menu:move_cursor_up()
...
function server_menu:move_cursor_down()
...
function server_menu:move_cursor_left_or_right()
...
function server_menu:set_cursor_position(cursor_position)

  self.cursor_position = cursor_position
  self.cursor_sprite:set_frame(0)  -- Restart the animation.
end

function server_menu:repeat_move_clouds()
...
-- Phase "select a file" --
---------------------------
function server_menu:init_phase_select_file()
...
function server_menu:key_pressed_phase_select_file(key)
  ...
  if key == "space" or key == "return" then
    ...
    elseif self.cursor_position == 4 then
      ...
      -- The user chooses a server.
      local slot = self.slots[self.cursor_position]
      if slot.offline then
        sol.audio.play_sound('wrong')
      elseif sol.game.exists(slot.file_name) then
        ...
        sol.timer.start(self, 700, function()
                          sol.menu.stop(self)
                          --slot.save:start()
          sol.main:start_game(slot.save,slot.server)
        end)
      else
        -- It's a new server: choose the player's name.
...
function server_menu:joypad_button_pressed_phase_select_file(button)

  return self:key_pressed_phase_select_file("space")
end

function server_menu:direction_pressed_phase_select_file(direction8)
...
function server_menu:draw_phase_select_file()
  ...
  -- Cursor.
  self:draw_server_cursor()

  -- Save numbers.
  for i = 1, 3 do
    self:draw_server_number(i)
  end
end
...
-- Phase "erase a file" --
--------------------------
function server_menu:init_phase_erase_file()
...
function server_menu:key_pressed_phase_erase_file(key)
  ...
  if key == "space" or key == "return" then
    ...
    elseif self.cursor_position > 0 and self.cursor_position <= 3 then
      -- The user chooses a server to delete.
      local slot = self.slots[self.cursor_position]
      if not sol.game.exists(slot.file_name) then
        -- The server doesn't exist: error sound.
        sol.audio.play_sound("wrong")
      else
        -- The server exists: confirm deletion.
        sol.audio.play_sound("ok")
...
function server_menu:joypad_button_pressed_phase_erase_file(button)

  return self:key_pressed_phase_erase_file("space")
end

function server_menu:direction_pressed_phase_erase_file(direction8)
...
function server_menu:draw_phase_erase_file()
...
-- Phase "Are you sure?" --
---------------------------
function server_menu:init_phase_confirm_erase()
...
function server_menu:key_pressed_phase_confirm_erase(key)
...
function server_menu:joypad_button_pressed_phase_confirm_erase(button)

  return self:key_pressed_phase_confirm_erase("space")
end

function server_menu:direction_pressed_phase_confirm_erase(direction8)
...
function server_menu:draw_phase_confirm_erase()

  -- Current server slot.
  self:draw_server(self.save_number_to_erase)
  self:draw_server_number(self.save_number_to_erase)
...
-- Phase "options" --
----------------------
function server_menu:init_phase_options()
...
function server_menu:key_pressed_phase_options(key)
...
function server_menu:joypad_button_pressed_phase_options(button)
  return self:key_pressed_phase_options("space")
end

function server_menu:direction_pressed_phase_options(direction8)
...
function server_menu:draw_phase_options()
...
function server_menu:set_options_cursor_position(position)
...
function server_menu:set_option_value(option, index)
...
function server_menu:reload_options_strings()
  ...
  self:set_bottom_buttons("selection_menu.back", nil)
  self:read_servers()  -- To update "- Empty -" mentions.
end

------------------------------
-- Phase "choose your name" --
------------------------------
function server_menu:init_phase_choose_name()
...
function server_menu:key_pressed_phase_choose_name(key)
...
function server_menu:joypad_button_pressed_phase_choose_name(button)

  return self:key_pressed_phase_choose_name("space")
end

function server_menu:direction_pressed_phase_choose_name(direction8)
...
function server_menu:draw_phase_choose_name()
...
function server_menu:add_letter_player_name()
...
function server_menu:validate_player_name()
  ...
  local save = self.slots[self.cursor_position].save
  self:set_initial_values(save)
  save:save()
  self:read_servers()
  return true
end

function server_menu:set_initial_values(save)
  save:set_value("player_name", self.player_name)
  initial_game:initialize_new_savegame(save)
end
